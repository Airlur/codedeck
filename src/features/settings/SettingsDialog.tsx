...
export function SettingsDialog({
  ...
  const [publishTokenExpireAt, setPublishTokenExpireAt] = useState<
    string | null
  >(null);
  ...
  const publicViewUrl = useMemo(() => {
    ...
    return new URL("/public", window.location.origin).toString();
  }, []);
  ...
  const runVerify = async () => {
    if (!adminPassword.trim()) {
      ...
      onShowToast("请输入管理员密码后再验证。", "warn", 2600);
      return;
    }
    ...
    try {
      ...
      setRepoMessage({
        ok: true,
        text: `验证成功，有效期到：${toDisplayTime(result.expiresAt)}`,
      });
      onShowToast("管理员密码验证成功，可以继续发布。", "success", 2600);
      ...
      setRepoMessage({ ok: false, text });
      onShowToast(text, "error", 4200);
    } finally {
      setVerifying(false);
    }
  };
  ...
  const runPublish = async () => {
    ...
    try {
      ...
      setRepoMessage({
        ok: true,
        text: shortCommit
          ? `发布成功：${shortCommit}。公开页链接：${publicViewUrl}`
          : `发布成功。公开页链接：${publicViewUrl}`,
      });
      setCommitMessage(defaultCommitMessage());
    } catch (error) {
      setPublishSucceeded(false);
  ...
  const copyPublicViewUrl = async () => {
    try {
      await navigator.clipboard.writeText(publicViewUrl);
      onShowToast("公开页链接已复制。", "success", 2400);
    } catch {
      onShowToast(`复制失败，请手动复制：${publicViewUrl}`, "warn", 4200);
    }
  };
      ...
      <div className="grid h-[min(560px,calc(70vh-2rem))] grid-cols-[180px_1fr] gap-0 overflow-hidden">
        <aside className="border-r border-slate-200 bg-slate-50 p-3">
          <nav className="space-y-1">
            <button
              type="button"
              className={navButtonClass("webdav")}
            ...
            <div className="flex h-full flex-col">
              <div className="mb-4 flex items-center justify-between">
                ...
                <div className="inline-flex items-center rounded-lg border border-slate-200 bg-slate-100 p-1">
                  <button
                    type="button"
                    className={webdavSwitchClass("config")}
                    onClick={() => setWebdavPanel("config")}
                  >
                    配置
                  </button>
                  <button
                    type="button"
                    className={webdavSwitchClass("history")}
                    onClick={() => setWebdavPanel("history")}
                  >
                    历史版本
                  </button>
                </div>
                        ...
                        <label className="text-sm text-slate-700">
                          ...
                          <div className="relative mt-1">
                            ...
                            <button
                              ...
                              onClick={() =>
                                setShowWebDavPassword((current) => !current)
                              }
                              title={
                  ...
                  <section className="rounded-xl border border-slate-200 p-4">
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <p className="text-xs text-slate-500">
                        每次推送时生成新备份，超出限制自动清理旧文件（最少5，最多 50）。
                      </p>
                      <div className="w-24">
                        <TextInput
                          type="number"
            ...
            <div className="flex h-full min-h-0 flex-col overflow-auto pr-1">
              <div className="space-y-5">
                <section>
                  ...
                  <p className="text-sm text-slate-600">
                    发布会把当前本地数据中的“公开工具”生成
                    `public/published.json`，提交到 GitHub
                    仓库，部署平台将自动重新部署。
                  </p>
                  <p className="mt-1 text-xs text-slate-500">
                    工具公开/私密可在编辑页「详情 / 发布可见性」里切换。
                  </p>
                  <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
                    发布完成后，请访问 `/public` 或 `/?mode=public`
                    查看公开快照。当前管理模式首页显示的仍然是本浏览器的本地/WebDAV
                    数据，不会直接切到公开快照。
                  </div>
                </section>
                ...
                <section className="space-y-3 rounded-xl border border-slate-200 p-4">
                  ...
                  {publishTokenExpireAt ? (
                    <div className="text-xs text-slate-500">
                      当前令牌有效期：{toDisplayTime(publishTokenExpireAt)}
                    </div>
                  ) : null}
                </section>
              ...
              {publishSucceeded ? (
                <div className="flex flex-wrap items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                  ...
                  <Button
                    variant="secondary"
                    className="border-slate-300 px-2 py-1 text-xs"
                  ...
                  <Button
                    variant="secondary"
                    className="border-slate-300 px-2 py-1 text-xs"
                    onClick={() => void copyPublicViewUrl()}
                  >
                    复制链接
                  </Button>
                </div>
