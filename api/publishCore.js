...
function getVerifyWindowMs() {
  const sec = clampInt(process.env.PUBLISH_VERIFY_WINDOW_SEC, DEFAULT_VERIFY_WINDOW_SEC, 60, 3600);
  return sec * 1000;
}

function getVerifyMaxAttempts() {
  return clampInt(process.env.PUBLISH_VERIFY_MAX_ATTEMPTS, DEFAULT_VERIFY_MAX_ATTEMPTS, 1, 20);
}

function getSessionTtlMs() {
  const sec = clampInt(process.env.PUBLISH_SESSION_TTL_SEC, DEFAULT_SESSION_TTL_SEC, 60, 7200);
  return sec * 1000;
}
...
function createTokenSecret(adminPassword) {
  const extra = String(process.env.PUBLISH_AUTH_SECRET ?? '').trim();
  if (extra) return extra;
  return hashText(`${adminPassword}|${process.env.GITHUB_TOKEN ?? ''}|codedeck_publish`);
}
...
function isRateLimited(now, ip) {
  cleanupExpiredAttempts(now);
  const row = verifyAttempts.get(ip);
  if (!row) return false;
  return row.count >= getVerifyMaxAttempts() && row.resetAt > now;
}

function recordVerifyFailure(now, ip) {
  const windowMs = getVerifyWindowMs();
  const row = verifyAttempts.get(ip);
...
async function handleVerify(body, headers) {
  ...
  if (!timingSafeEqualText(providedPassword, adminPassword)) {
    recordVerifyFailure(now, ip);
    const remaining = Math.max(0, getVerifyMaxAttempts() - (verifyAttempts.get(ip)?.count ?? 0));
  ...
  const secret = createTokenSecret(adminPassword);
...
function verifyPublishToken(rawToken, headers) {
  const adminPassword = String(process.env.ADMIN_PASSWORD ?? '');
  if (!adminPassword) {
...
async function handlePublish(body, headers) {
  const authResult = verifyPublishToken(body.token, headers);
  if (!authResult.ok) {
    return authResult.status;
  }

  const token = String(process.env.GITHUB_TOKEN ?? '').trim();
  const repoSlug = String(process.env.GITHUB_REPO ?? '').trim();
  const branch = String(process.env.GITHUB_BRANCH ?? DEFAULT_BRANCH).trim() || DEFAULT_BRANCH;
  const publishPath =
    String(process.env.GITHUB_PUBLISH_PATH ?? DEFAULT_PUBLISH_PATH).trim() || DEFAULT_PUBLISH_PATH;
...
export async function processPublishRequest({ method, headers, rawBody }) {
  ...
  if (action === 'verify') {
    return handleVerify(parsedBody, headers || {});
  }

  if (action === 'publish') {
    return handlePublish(parsedBody, headers || {});
  }
